...
func TestParseDSN(t *testing.T) {
	...
	if err != nil {
		log.DefaultLogger.ErrorFields("mysql.ParseDSN failed", zap.Error(err))
		return
	}
	log.DefaultLogger.InfoFields("mysql.ParseDSN success", zap.Reflect("cfg", cfg))
}

func TestCreateEmpty(t *testing.T) {
	if err := getDBClient(); err != nil {
		log.DefaultLogger.ErrorFields("New gorm db client failed", zap.Error(err))
		return
	}
	...
	if err := dbClient.Table("test_database.test_table").Create(data).Error; err != nil {
		log.DefaultLogger.ErrorFields("gorm.Create failed", zap.Error(err))
		return
	}
}
...
func TestWhere(t *testing.T) {
	...
	if err := dbClient.Table("test_database.test_table").Where(&TestTable{ID: 0, CharField: ""}).Find(&results).Error; err != nil {
		log.DefaultLogger.ErrorFields("gorm.Find failed", zap.Error(err))
		return
	}
