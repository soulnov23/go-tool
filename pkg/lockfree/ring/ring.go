...
const (
	...
	// uint64大小，atomic.Uint64只是给uint64补上了不可复制和8字节对齐的约束，两者同宽
	// 所有填充都由它算出，不把字段大小写死成字面量
	uint64Size = unsafe.Sizeof(uint64(0))
	// 最小容量
	minCapacity = 2
	// 最大容量，uint64范围内最大的2的幂，再往上向上取整就会溢出
	maxCapacity = uint64(1) << 63
	// node补齐到整缓存行所需的填充字节数
	// any在32位平台上只占8字节，不能按64位平台把有效字段大小写死成24
	nodePadSize = cacheLinePadSize - uint64Size - unsafe.Sizeof(any(nil))
)
...
type node struct {
	...
	value any               // 节点存储的值
	_     [nodePadSize]byte // 填充至整缓存行，避免相邻节点伪共享
}
...
type Queue struct {
	...
	_        [cacheLinePadSize - uint64Size*2]byte // 填充至缓存行大小
	/*----------------CacheLine----------------*/
	head atomic.Uint64                       // 出队票号发号器
	_    [cacheLinePadSize - uint64Size]byte // 避免伪共享
	/*----------------CacheLine----------------*/
...
// New 创建指定容量的环形队列
// 容量会自动调整为大于等于输入值的最小2的幂，并钳制在[minCapacity, maxCapacity]区间内
...
// roundUpToPower2 将输入值调整为大于等于它的最小2的幂，结果钳制在[minCapacity, maxCapacity]区间内
// 容量的下限收口在这里，调用方不需要再自己判断minCapacity
// 低于minCapacity时抬到minCapacity；超过maxCapacity时无法向上取整，饱和到maxCapacity，避免溢出后返回0
...
func (queue *Queue) Enqueue(value any) {
	...
	for {
		slot := &queue.nodes[tail&queue.mask]
		// 转成有符号数比较，利用二进制补码天然处理uint64回绕
		diff := int64(slot.seq.Load() - tail)
		if diff < 0 {
...
func (queue *Queue) Dequeue() any {
	...
	for {
		slot := &queue.nodes[head&queue.mask]
		// 节点已写入的标志是seq推进到了head+1
		diff := int64(slot.seq.Load() - (head + 1))
		if diff < 0 {
		...
		if diff == 0 && queue.head.CompareAndSwap(head, head+1) {
			...
			slot.seq.Store(head + queue.capacity)
			return value
		}
